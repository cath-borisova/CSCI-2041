...
(* THING. A Lisp object.
 ...
 (*
   YOUR CODE FOR NUMBER GOES HERE!
 *)
primitive "number" 
  (fun args env ->
    match args with
        Cons(arg, Nil) -> 
          (match (evaluating arg env) with
           Number(_) -> tee |
          _ -> Nil) |
        _ -> oops "NUMBER expected 1 thing");;
 
 (*
   YOUR CODE FOR IMPLY GOES HERE!
 *)
 primitive "imply" 
  (fun args env ->
    let rec implying args =
      match args with 
        ...
        Cons(arg, args) -> 
          ...
          _ ->  oops "IMPLY expected 0 or more THINGs"
        in implying args) ;;
 
 (*
   YOUR CODE FOR LET GOES HERE!
 *)
 primitive "let"
 (fun args env ->
   match args
   with Cons (left, Cons (mid, Cons(right, Nil))) ->
          (match left
           with Symbol name ->
                  global := envPut name (evaluating mid env) (! global) ;
                  evaluating right env|
                _ ->
                  oops "LET expected a SYMBOL") |
        _ ->
          oops "LET expected 3 args") ;;
 end ;;
